#include "CameraScreenVM.h"
#include <QDebug>
#include <QtConcurrent/QtConcurrent>

CameraScreenVM::CameraScreenVM(QObject *parent)
    ...
    , decoding(false)
{
    ...
    connect(&decoder, &QZXing::decodingStarted,
            this, &CameraScreenVM::handleDecodingStarted);
    connect(&decoder, &QZXing::decodingFinished,
            this, &CameraScreenVM::handleDecodingFinished);
    connect(&decoder, &QZXing::tagFound,
            this, &CameraScreenVM::handleTagFound);

    Bus *l_bus = Bus::getInstance();
    connect(this, &CameraScreenVM::tagFound,l_bus, &Bus::decodedInfo);
}
...
void CameraScreenVM::handleDecodingStarted()
{
    decoding = true;
...
void CameraScreenVM::handleDecodingFinished(bool succeeded)
{
    decoding = false;
...
void CameraScreenVM::handleTagFound(QString tag)
{
//    qDebug() << "handleTagFound";
    emit tagFound(tag);
}

QVideoFilterRunnable * CameraScreenVM::createFilterRunnable()
{
...
QImage CameraScreenVM::fromBGRAtoARGB(uchar * data, QSize size, QVideoFrame::PixelFormat pixelFormat)
{
...
QZXingFilterRunnable::QZXingFilterRunnable(CameraScreenVM * filter)
    : QObject(nullptr)
    , filter(filter)
{
...
//    qDebug() << "Future: Going to process frame: " << i;
    ...
    if(image.isNull())
        image = CameraScreenVM::fromBGRAtoARGB((uchar*)videoFrame.data.data(), videoFrame.size, videoFrame.pixelFormat);
