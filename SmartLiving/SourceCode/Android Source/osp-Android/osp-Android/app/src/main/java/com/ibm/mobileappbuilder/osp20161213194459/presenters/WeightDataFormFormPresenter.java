...
package com.ibm.mobileappbuilder.osp20161213194459.presenters;
...
public class WeightDataFormFormPresenter extends BaseFormPresenter<WeightdbDSSchemaItem> {

    private final CrudDatasource<WeightdbDSSchemaItem> datasource;

    public WeightDataFormFormPresenter(CrudDatasource<WeightdbDSSchemaItem> datasource, FormView<WeightdbDSSchemaItem> view){
        super(view);
        this.datasource = datasource;
    }

    @Override
    public void deleteItem(WeightdbDSSchemaItem item) {
        datasource.deleteItem(item, new OnItemDeletedListener());
    }

    @Override
    public void save(WeightdbDSSchemaItem item) {
        // validate
        if (validate(item)){
    ...
    @Override
    public void create(WeightdbDSSchemaItem item) {
        // validate
        if (validate(item)){
    ...
    private class OnItemDeletedListener extends ShowingErrorOnFailureListener {
        @Override
        public void onSuccess(WeightdbDSSchemaItem  item) {
    ...
    private abstract class ShowingErrorOnFailureListener implements Datasource.Listener<WeightdbDSSchemaItem > {
        @Override
    ...
    private void logCrudAction(String action, String id) {
      ...
      paramsMap.put("entity", "WeightdbDSSchemaItem");
